...
def plot_signal(x, x_pred):
    ...
    plt.plot(np.arange(0, x.shape[0]), x, linewidth=1.0, label='Target')
    plt.plot(np.arange(0, x_pred.shape[0]), x_pred, linewidth=1.0, label='Prediction')
    plt.xlabel('Time')
